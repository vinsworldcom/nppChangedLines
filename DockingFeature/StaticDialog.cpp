...
void StaticDialog::create(int dialogID, bool isRTL)
{
    ...
    if (!_hSelf)
    {
        ...
        char errMsg[256];
        sprintf(errMsg, "CreateDialogParam() return NULL.\rGetLastError() == %lu\rHandle = %p\rResourceID = %i", err,(void*)GetModuleHandle(TEXT("ChangedLines.dll")),dialogID);
...
INT_PTR CALLBACK StaticDialog::dlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            ::SendMessage( GetDlgItem( hwnd, IDC_CHK_ENABLED ), BM_SETCHECK, ( LPARAM )( g_enabled ? 1 : 0 ), 0 );
